...
int flag, tbl;
{
    ...
    init_jutil(msg_get(cd, 69, default_message[69], xjutil->lang),
...
retry:
	...
	if(word_searched[k].hindo == -1){
	    ...
	    sprintf(c,"%c%-3d  ",(word_searched[k].ima ? '*':' '),
		    word_searched[k].hindo );
	}
	c += 6;
	if(!(word_searched[k].int_ima && word_searched[k].int_hindo == -1)){
		...
		sprintf(c,"(%c%-3d)  ", (word_searched[k].int_ima)? '*':' ',
			word_searched[k].int_hindo );
	    }
	} else {
		sprintf(c,"        ");
	}
    ...
    k =  xw_select_one_element_keytable(info, rcnt, 0,
					msg_get(cd, 69, default_message[69],
						xjutil->lang),
					6, table_t, jutil_table[0],
					main_table[tbl],  NULL);
    if (k == -1 || k == -3) {
	return(NULL);
    }
