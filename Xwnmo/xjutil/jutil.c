...
"NULL", "NULL", "NULL", "NULL", "NULL", "NULL", "NULL"};
    ...
    c = xw_select_one_element_call(cccmsg,(sizeof(ccc) / sizeof(char *)), init,
			           msg_get(cd, 9, default_message[9],
					   xjutil->lang));
    if (c == -1 || c == -3) {
        return;
    }
	...
	sprintf(message1 , "%s %s %s %d %s",
		...
		msg_get(cd, 10 + c, default_message[10 + c], xjutil->lang),
		msg_get(cd, 31, default_message[31], xjutil->lang),
		paraary[c], msg_get(cd, 32, default_message[32], xjutil->lang));
	sprintf(message2 , "%s",
		msg_get(cd, 33, default_message[33], xjutil->lang));
    }
    mes[0] = message2;
    init_jutil(message1, msg_get(cd, 8, default_message[8], xjutil->lang),
	       1, &mes, NULL);
Retry:
...
dicinfoout()
{
    ...
    if (init < 0 || init > (cnt - 1)) init = 0;
    init = xw_select_one_element_keytable(info, cnt - 1, init,
					  msg_get(cd, 36, default_message[36],
						  xjutil->lang),
					  3, table_t, jutil_table[1],
					  main_table[8], comment);
    return;
}
...
select_one_dict9()
{
    ...
    if (init < 0 || init > dic_list_size) init = 0;
    init = xw_select_one_element_call(buf_ptr, dic_list_size, init,
				      msg_get(cd, 41, default_message[41],
					      xjutil->lang));
    return;
}
...
#ifdef	CHINESE
	if(!strncmp(js_get_lang(env), LANG_CN, 5) ||
	   !strncmp(js_get_lang(env), LANG_TW, 5))
...
Dicadd_Retry:
    ...
    if (((id = dic_add_e(cur_env,jishopath,hfname,0,prio,rdonly, hrdonly, NULL,
			 NULL, yes_or_no_or_newline, print_msg_getc)) == -1) &&
	(wnn_errorno != -1)){
	errorkeyin();  
    }
    update_dic_list();
	...
	if ((dic_add_e(cur_reverse_env->env, jishopath,hfname, 1,prio,rdonly,
		       hrdonly, NULL, NULL,
