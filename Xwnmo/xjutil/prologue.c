/*
 * $Id: prologue.c,v 1.1.1.1 2000-01-16 05:07:53 ura Exp $
 */
...
open_romkan()
{
    ...
    cur_rk_table = romkan_table_init(NULL, rkfile_name, NULL,
				     bytcntfn, NULL, 0);
...
char *host_name;
{
	...
	for (j = js_list; j; j = j->next) {
	    if (js_list->host_name == NULL) return(js_list->js);
	}
	return(NULL);
    } else {
	for (j = js_list; j; j = j->next) {
	    if (!strcmp(js_list->host_name, host_name)) return(js_list->js);
	}
	return(NULL);
    }
}
...
char *host_name;
{
    ...
    if (!(j = (js_list_str *)Malloc(sizeof(js_list_str)))) return(-1);
    j->js = js;
...
connect_server()
{
	...
	if (!p->host_name || !*p->host_name) {
		...
		if (js = find_same_js(p->host_name)) goto NEXT_STEP1;
...
NEXT_STEP1:
	...
	if (!p->host_name || !*p->host_name) {
	    if (machine_n = _wnn_get_machine_of_serv_defs(xjutil->lang)) {
		p->host_name = alloc_and_copy(machine_n);
		if (js = find_same_js(p->host_name)) goto NEXT_STEP2;
		if ((js = js_open_lang(p->host_name, xjutil->lang,
				       WNN_TIMEOUT)) == NULL) {
		    p->host_name = NULL;
		}
