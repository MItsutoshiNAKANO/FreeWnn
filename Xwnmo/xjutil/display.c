/*
 * $Id: display.c,v 1.1.1.1 2000-01-16 05:07:53 ura Exp $
 */
...
int which;
{
	...
	XwcDrawImageString(dpy, cur_text->wn[which], cur_fs->fs, cur_root->gc,
...
int wc_len;
{
	...
	*JW_att = flg;
    ...
    JWOutput(text->w, cur_fs->fs,
	     ((flg & REV_FLAG)? cur_root->reversegc: cur_root->gc),
...
int which;
{
    XwcDrawImageString(dpy, tx->wn[which], cur_fs->fs, cur_root->gc,
		       0, FontAscent, &dol_wchar_t, 1);
    XFlush(dpy);
}
