/*
 * $Id: xw_touroku.c,v 1.1.1.1 2000-01-16 05:07:53 ura Exp $
 */
...
unsigned char *mes1[];
{
    ...
    changecolor_box(jutil->title, cur_root->fg, cur_root->bg, cur_root->fg,
		    BoxAll);
    changecolor_box(jutil->button[CANCEL_W], cur_root->fg, cur_root->bg,
		    cur_root->fg, BoxAll);
    changecolor_box(jutil->button[EXEC_W], cur_root->fg, cur_root->bg,
		    cur_root->fg, BoxAll);
	...
	moveresize_box(box, (FontWidth - IN_BORDER),
		       (FontHeight * (3 + i * 2) - IN_BORDER),
		       (FontWidth * (len_t = strlen((char*)box->string))
			+ IN_BORDER * 2),
		       (FontHeight + IN_BORDER * 2));
	...
	XMoveResizeWindow(dpy, text->wp,
			  (FontWidth * max_mes_t), (FontHeight * (3 + i * 2)),
			  (FontWidth * text->max_columns), FontHeight);
    ...
    moveresize_box(jutil->rk_mode,
		   ...
		   (FontHeight + IN_BORDER * 2));
    changecolor_box(jutil->rk_mode, cur_root->fg, cur_root->bg, cur_root->fg,
		    BoxAll);
    decide_position(width, height, cur_root->bw, &x_cood, &y_cood);
#ifdef nodef
    XQueryPointer(dpy, cur_root->root_window, &root, &child,
			&save_x, &save_y, &win_x, &win_y, &keys_buttons);
    jutil->save_p.x = (short)save_x;
    jutil->save_p.y = (short)save_y;
    XWarpPointer(dpy, None, cur_root->root_window, 0, 0, 0, 0,
		 x_cood + FontWidth * 2, y_cood + FontHeight);
#endif

    XMoveResizeWindow(dpy, jutil->w, x_cood - 2, y_cood - 2, width, height);
    set_wm_properties(jutil->w, x_cood - 2, y_cood - 2, width, height,
		      CenterGravity, NULL, NULL, 0, 0, 0, 0, 0);
