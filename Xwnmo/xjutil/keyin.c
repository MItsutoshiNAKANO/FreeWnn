/*
 * $Id: keyin.c,v 1.1.1.1 2000-01-16 05:07:53 ura Exp $
 */
...
get_env()
{
    ...
    XGetWindowProperty(dpy,
		xjutil->root_pointer[xjutil->default_screen]->root_window,
		atom_env, 0L, 1000000L, False,XA_STRING, &actual_type,
...
{
	if (ev->window == cur_root->ichi->w ||
	    ev->window == cur_root->jutil->w) {
		xjutil->sel_ret = -2;
		return(0);
	}
	return(1);
...
register int	*buff;
{
	...
	if (event.type != ClientMessage && !cur_root) continue;
	switch (event.type) {
	    case KeyPress:
		xjutil->sel_button = 0;
