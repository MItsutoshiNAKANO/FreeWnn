/*
 * $Id: touroku.c,v 1.1.1.1 2000-01-16 05:07:53 ura Exp $
 */
...
int mode;
{
	...
	changecolor_box(jutil->mes_button[mode], 0, 0, cur_root->bg,
			BoxBorderPixel);
...
int mode;
{
	...
	changecolor_box(jutil->mes_button[mode], 0, 0, cur_root->fg,
			BoxBorderPixel);
...
touroku()
{
	...
	if((dicinfo[i].type == WNN_UD_DICT ||
	    dicinfo[i].type == WNN_REV_DICT ||
	    dicinfo[i].type == CWNN_REV_DICT) && dicinfo[i].rw == 0){
    ...
    if ((hinsi = get_default_hinsi(save_text)) != -1) save_hinsi = hinsi;
...
repeat:
	...
	switch(touroku_mode) {
		...
		unfreeze_messages();
		if (Strlen(yomibuf) > 0) yomi_set = 1;
		...
		JWMline_clear(0);
		kk_cursor_invisible();
		ud_no = xw_select_one_element_call(dict_name,size,current,
						   msg_get(cd, 98,
							   default_message[98],
							   xjutil->lang));
		if (ud_no == -1) {
...
do_touroku:
    ...
    if(js_word_add(cur_env, current_dic, yomibuf, hani_buffer,
		   "", hinsi, 0) == -1){
...
t_markset()
{
	...
	call_t_redraw_move_1(find_end_of_tango(c_b->t_c_p), c_b->t_c_p,
			     find_end_of_tango(c_b->t_c_p), 0, 0, 0, 2);
        kk_cursor_invisible();
        return(0);
    }else{
