/*
 * $Id: xselectele.c,v 1.1.1.1 2000-01-16 05:07:53 ura Exp $
 */
...
char *title;
{
    ...
    for (;;) {
	if ((ret = xw_select_one_element(c,kosuu1,init, title, SENTAKU,
					 main_table[4], in)) != BUFFER_IN_CONT)
	    return(ret);
	in = keyin();
    }
}
...
int
xw_select_one_element_keytable(c,kosuu1,init,title, bcnt,btable_t,b_tbl,
			       key_tbl,comment)
char **c;
int kosuu1;
int init;
...
char *comment;
{
  ...
  kosuu = kosuu1;
  init_ichiran(c, kosuu1,init, title, (unsigned char *)"",
	       (unsigned char *)"", comment, 0, SENTAKU);
  for(;;) {
TOP:
    xjutil->sel_ret = -1;
    c1 = keyin() ;
...
int kdiccom()
{
    ...
    init_jutil(msg_get(cd, 39, default_message[39], xjutil->lang),
...
kworddel_op()
{
	...
	if (ichi->buf[i][del_point] != 'D') continue;
	...
	if(yes_or_no(buf) == 1){
	    if(js_word_delete(cur_env, word_searched[i].dic_no,
			      word_searched[i].serial) == -1){
		if_dead_disconnect(cur_env, -1);
		errorkeyin();
	    }
	}
...
int hindo;
{
    if(js_hindo_set(cur_env, word_searched[cc].dic_no, word_searched[cc].serial,
		    ima, hindo) == -1) {
	if_dead_disconnect(cur_env, -1);
	errorkeyin();
	return(0);
...
kwordhindo()
{
    ...
    if (dicinfo[find_dic_by_no(word_searched[cc].dic_no)].hindo_rw ==
	WNN_DIC_RDONLY){
	print_msg_getc(msg_get(cd, 118, default_message[118], xjutil->lang),
		       NULL, NULL, NULL);
	return(0);
    }
    ...
    sprintf(message, "%s %d %s",
	    msg_get(cd, 119, default_message[119], xjutil->lang),
	    cur_hindo,
	    msg_get(cd, 32, default_message[32], xjutil->lang));

    mes[0] = msg_get(cd, 33, default_message[33], xjutil->lang);
    init_jutil(message, msg_get(cd, 8, default_message[8], xjutil->lang),
	       1, &mes, NULL);
Retry:
    cur_text->max_pos = 127;
    clr_line_all();
	...
	print_msg_getc("%s", msg_get(cd, 35, default_message[35],
		       xjutil->lang), NULL, NULL);
	goto Retry;
    }
