/*
 * $Id: xresource.c,v 1.1.1.1 2000-01-16 05:07:53 ura Exp $
 */
...
void	createxres()		/* CREATE X RESources */
{
    ...
    if (XAllocColorCells(dpy, dcmap, False, plane, 0,
			 pixel[cmflag], PIX_MAX)) {   
	for (pix_no = 0; pix_no < PIX_MAX; pix_no++) {
	    XStoreNamedColor(dpy, dcmap, name[cmflag][pix_no],
			     pixel[cmflag][pix_no], flags);
	}
    } else {
...
void	mktwin()		/* MaKe Test WINdow */
{
    ...
    window1.win = XCreateSimpleWindow(dpy, rwin, W1_X, W1_Y, W1_WIDTH, W1_HEIGHT,
				      W_BORDER, pixel[cmflag][2], pixel[cmflag][1]);
    window2.win = XCreateSimpleWindow(dpy, window1.win, W23_X, W2_Y,
				      W2_WIDTH, W23_HEIGHT, W_BORDER, pixel[cmflag][2], pixel[cmflag][1]);
    window3.win = XCreateSimpleWindow(dpy, window1.win, W23_X, W3_Y,
				      W3_WIDTH, W23_HEIGHT, W_BORDER, pixel[cmflag][2], pixel[cmflag][1]);
    ufocus = window3.win;
    window1.name = XCreateSimpleWindow(dpy, window1.win, WIN_ITV, CALCY(W1_HEIGHT, NW_HEIGHT),
				       NW_WIDTH, NW_HEIGHT, W_BORDER, pixel[cmflag][1], pixel[cmflag][1]);
...
void	mkmwin()		/* MaKe Menu WINdow */
{
    ...
    mroot = XCreateSimpleWindow(dpy, rwin, MROOT_X, MROOT_Y,
				mroot_width, mroot_height,
				M_BORDER, pixel[cmflag][2], pixel[cmflag][1]);
    ...
    XSetWMProperties(dpy, mroot, &win_name, &icon_name, NULL, 0,
		     &hints, &wm_hints, &class_hints);
    strflag = MB;
    strmode = XCreateSimpleWindow (dpy, mroot, MSUB_WIDTH * 11, 0,
				   MSUB_WIDTH, MSUB_HEIGHT, 0,
				   pixel[cmflag][2], pixel[cmflag][1]);
