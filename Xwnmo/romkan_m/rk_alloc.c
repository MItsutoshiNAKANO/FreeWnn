/*
 * $Id: rk_alloc.c,v 1.1.1.1 2000-01-16 05:07:53 ura Exp $
 */
...
int	size;
{
    if ((ptr->size = size) > 0) {
	if ((ptr->org = ptr->next = (char *)malloc(ptr->size)) == NULL ||
	    (ptr->tmp = (char *)malloc(ptr->size)) == NULL) {
		ERRALLOC(0);
	}
    } else {
	ptr->org = NULL;
...
int	size;
{
	...
	if ((size + 2) > ptr->size) {
		...
		if ((ptr->org = (char *)malloc(ptr->size = size + 2)) == NULL ||
		    (ptr->tmp = (char *)malloc(ptr->size = size + 2)) == NULL) {
			ERRALLOC(0);
		}
	    } else {
		if ((ptr->org =
		    ...
		    (ptr->tmp =
		    (char *)realloc(ptr->tmp, ptr->size = size + 2)) == NULL) {
			ERRALLOC(0);
		}
	    }
	}
