/*
 * $Id: rk_read.c,v 1.1.1.1 2000-01-16 05:07:53 ura Exp $
 */
...
fast	int	n;	/* V3.1 */
{
	...
	*(c_r_t->rk_hensuu.next = c_r_t->rk_hensuu.org) = EOLTTR;
	...
	/* 88/06/11/ V3.1 */
	c_r_t->rk_hyobuf.next = c_r_t->rk_hyobuf.org;
	while (c_r_t->rk_hyobuf.next =
	    readln_ustrtoltr(c_r_t->rk_hyobuf.next, (ltrbufptr = ltrbuf))) {
 ...
 /* rsltyp: 0=文字項 1=文字列連 2=データ連 3=機能項 4=宣言項 -1=存在せず */
	...
	for (i = 0, hensuptr = c_r_t->rk_hensuu.point; i < c_r_t->rk_hensuu.count;
							i++, hensuptr++) {
		if (hensuptr->regdflg == 0) ERRHYO(1);	/* V3.1 */

		check_and_realloc_for_hensuudef(&(c_r_t->rk_hensuudef),
				j = c_r_t->rk_hensuudef.count, RK_HENSUUDEF_MAX_LOT);
...
int	flg;
{
	...
	for (n = 0; n < hentptr->count; n++){
		if(ltrcmp(hentptr->point[n].name, name)) continue;
		 /* found */
		if(flg == -1 && hentptr->point[n].regdflg != 0) ERRLIN(10);
		if(flg == 1 && hentptr->point[n].curlinflg == 0) ERRLIN(5);
	...
	check_and_realloc_for_hensuu(hentptr, n, name,
