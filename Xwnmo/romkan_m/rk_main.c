/*
 * $Id: rk_main.c,v 1.1.1.1 2000-01-16 05:07:53 ura Exp $
 */
...
#ifdef nodef
static letter	*ltrncpy(lp1, lp2, n) /* 末尾に'EOLTTR'をセット */
letter	*lp1, *lp2;
int	n;	
{
	fast	letter	*org;
...
letter	romkan_next()
{
	...
	if( (letter)EOF == (in = (*(cur_rk_table->keyin_method))() )) return(LTREOF);
	c = (in &= 0xff);
	n = (*(cur_rk_table->kbytcnt_method))(&c);
	for(i = 1; i < n; i++) in = (in << 8) + ((*(cur_rk_table->keyin_method))() & 0xff);
	return(in);
}
...
Romkan *
romkan_init3
(delchr_, nisedl_, delchr2_, flags_)
...
romkan_restart()
{
	...
	*/
	...
	if (NULL != romkan_table_init(rk_t, rk_t->prv_modfnm,
				      rk_t->keyin_method,
				      rk_t->bytcnt_method,
				      rk_t->kbytcnt_method,
				      rk_t->flags)) {
