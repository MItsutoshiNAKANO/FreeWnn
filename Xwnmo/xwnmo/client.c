...
set_server_name()
{
	...
	if (!(def_reverse_servername =
	      alloc_and_copy(root_def_reverse_servername))) {
	    if (def_servername) {
		Free(def_servername);
		def_servername = NULL;
	    }
...
int rev;
{
	...
	if (pp != p && !strcmp(name, cc->user_name)) {
		...
		if (!p->host_name && !pp->host_name) {
			...
			return(1);
		    } else if (*p->env_name_str && *pp->env_name_str &&
                      !strcmp(p->env_name_str, pp->env_name_str)) {
			return(1);
		    }
		} else if (p->host_name && pp->host_name &&
			   !strcmp(p->host_name, pp->host_name)) {
		    if (!*p->env_name_str && !*pp->env_name_str) {
			return(1);
		    } else if (*p->env_name_str && *pp->env_name_str &&
                      !strcmp(p->env_name_str, pp->env_name_str)) {
