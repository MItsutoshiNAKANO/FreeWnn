/*
 * $Id: inspect.c,v 1.1.1.1 2000-01-16 05:07:56 ura Exp $
 */
...
int in;
{
	...
	if (!xim->cur_j_c_root->inspect->map) return(BUFFER_IN_CONT);
	if (xim->sel_ret == -2) {
	    xim->sel_ret = -1;
            end_inspect();
	    select_step = 0;
...
int in;
{
	...
	kanji_len = jl_kanji_len(bun_data_, bun_no, bun_no+1)
	...
	sprintf(buf + strlen(buf), "%s?",
		msg_get(cd, 9, default_message[9], lang));
	step++;
    }
...
int in;
{
	...
	set_escape_code(buf);	/* ADD KURI */
	sprintf(buf + strlen(buf), "%s",
		msg_get(cd, 15, default_message[15], lang));
	step++;
    }
    if((ret = yes_or_no(buf, in)) != BUFFER_IN_CONT) {
	if (ret == 1) {
