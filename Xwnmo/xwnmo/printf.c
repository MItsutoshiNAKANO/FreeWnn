/*
 * $Id: printf.c,v 1.1.1.1 2000-01-16 05:07:56 ura Exp $
 */
...
w_char w;
{
	...
	if(wch == 0x7f)w_buffer[w_maxbuf++] =(w_char)('?');
	else w_buffer[w_maxbuf++] =(w_char)(wch + 'A' - 1);
    }else{
	if (print_out_func) {
...
#ifdef	XJUTIL
	ret = xw_read(tmp_buf);
	if (ret != -1 && cur_root->yes_no->map) break;
#else	/* XJUTIL */
	ret = get_yes_no_event();
	if (ret && cur_p->yes_no->map) break;
	xim->sel_ret = -1;
#endif	/* XJUTIL */
    }
    end_yes_or_no();
...
#ifdef	XJUTIL
	xw_read(tmp_buf);
	if (cur_root->yes_no->map && cur_root->yes_no->exp) break;
#else	/* XJUTIL */
	get_yes_no_event();
