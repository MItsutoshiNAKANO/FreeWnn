/*
 * $Id: change.c,v 1.1.1.1 2000-01-16 05:07:53 ura Exp $
 */
...
int cur_is_world;
{
	...
	if (i < xc->lang_num) continue;
	if ((ret = add_lang_env(xc, pp, &new)) == -2) {
	    return(-1);
	} else if (ret == -1) {
	    if (!cur_is_world) return(-1);
	} else {
	    xl = xc->xl[ret];
	    if (new != 0) {
		xc->lang_num++;
...
register XIMClientRec *xc;
{
    ...
    XGetStandardColormap(dpy, xc->root_pointer->root_window, &cmap,
			 xc->pe.std_colormap);
