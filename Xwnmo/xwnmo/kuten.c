/*
 * $Id: kuten.c,v 1.1.1.1 2000-01-16 05:07:56 ura Exp $
 */
...
char *lang;
{
	...
	if (multi > 1) {
	    sprintf(kuten_title, "%s %d.%2d %s", 
		msg_get(cd, MES1_NUM, default_message[MES1_NUM], lang), cs, ku,
		msg_get(cd, MES0_NUM, default_message[MES0_NUM], lang));
	} else {
	    sprintf(kuten_title, "%s %2d %s", 
		msg_get(cd, MES1_NUM, default_message[MES1_NUM], lang), ku,
		msg_get(cd, MES0_NUM, default_message[MES0_NUM], lang));
	}
	kuten_ichi_step++;
    }
    if((i = xw_select_one_element(buf_ptr, MAX_TEN_LENGTH, 0, kuten_title,
...
start:
	...
	(unsigned char *)msg_get(cd, MES3_NUM, default_message[MES3_NUM], lang),
	NULL, os+5, NYUURYOKU) == -1) {
#ifndef XJUTIL
	    c_c_sv = 0;
#endif /* !XJUTIL */
	    RING(-1);
...
#ifdef XJUTIL
	if (!cur_root->ichi->map) return(BUFFER_IN_CONT);
#else /* XJUTIL */
	if (!xim->cur_j_c_root->ichi->map) return(BUFFER_IN_CONT);
#endif /* XJUTIL */
	if (SEL_RET == -2) {
	    end_ichiran();
	    SEL_RET = -1;
	...
	if ((c <= '9' && c >= '0') || c == '.') {
		...
		if (mode >= 0) {
			...
			if ((c_p - mode) <= 4 && c != '.') {
			    buffer[c_p++] = c;
			    goto RET;
			} else if (mode == 1 && t_c >= '0' && t_c <= '2' &&
				   valid_cs[t_c-'0'] && (c_p - mode) <= 3 &&
				   (c_p - mode) >= 2 && c == '.') {
			    mode1 = c_p;
			    buffer[c_p++] = c;
			    goto RET;
	    ...
	    if (os > 0) {
		if ((mode1 >= 0 && (buffer[c_p - 1] != '.')) ||
		    (mode1 == -1 && mode >= 0 && (c_p - mode) >= 2) ||
		    (mode == -1 && c_p >= 3)) {
		    if (mode1 >= 0 || (mode == 1 && (c_p - mode) >= 4)) {
			cs = buffer[0] - '0';
			if (valid_cs[cs] == 0) RING(BUFFER_IN_CONT);
			start_p = &buffer[os];
		    } else {
			cs = 0;
			start_p = buffer;
		    ...
		    if ((!(sscanf(start_p , "%d.%d" , &ret1 , &ret2 ) == 2) && 
			 ...
			 !(((mode == -1)? (c_p == 3): (c_p == os+3)) && 
			   sscanf(start_p, "%1d%2d",&ret1 , &ret2) == 2))
		    ...
		    if (mode1 >= 0 && (buffer[c_p - 1] == '.')) {
			if (sscanf(&buffer[os] , "%d." , &ret1 ) != 1 ||
			    (ret1 <= 0 || ret1 > 94 )) {
			    RING(BUFFER_IN_CONT);
			}
	 	    } else if (mode == -1 && (c_p == 1 || c_p == 2)) {
			if (sscanf(&buffer[0] , "%d" , &ret1 ) != 1 ||
			    (ret1 <= 0 || ret1 > 94 )) {
			    RING(BUFFER_IN_CONT);
			}
		    } else {
