/*
 * $Id: readximrc.c,v 1.1.1.1 2000-01-16 05:07:57 ura Exp $
 */
...
int	(*expand_filenm)();
{
	...
	if (comment_char(*buf) ||
	    (n = sscanf(buf, "%s %s %s", entrynm, s[0],s[1])) < 1) {
		continue;
	}
	error = 0;
	switch(*(entrynm + 3)) {
	case 'l':
		...
		if (n > 1) {
		    ...
		    if (!(rr->name = alloc_and_copy(s[0]))) goto CLOSE_RET;
		    rr->next = NULL;
		} else
		    error = 1;
	    } else
	...
	case 'u':
		...
		if (n > 2) {
			...
			if (!strcmp(ld->lang, s[0])) break;
		    }
		    if (!ld) {
			print_out2(
		"In ximrc \"%s\", I don't support the lang \"%s\".",
				   filenm, s[0]);
		    }
		    if ((*expand_filenm)(s[1]) != 0) {
			print_out1("In ximrc \"%s\", I could not expand %s.",
				   s[1]);
		    }
...
read_ximrc()
{
    ...
    return(-1);
OK_RET:
    if (!(ximrc_file = alloc_and_copy(tmp_xim_rc_file))) return(-1);
    return(0);
}
...
read_ximconf()
{
	...
	if (!ret) break;
	if (comment_char(*buf) ||
	    (n = sscanf(buf, "%s %s %s %s %s %s",
			s[0],s[1],s[2],s[3],s[4],s[5])) < 1) {
		continue;
	}
	if (!strcmp(s[0], "setdefaultlocale")) {
	    if (n < 2) goto ARG_ERR;
	    if (!(def_locale = alloc_and_copy(s[1]))) goto CLOSE_RET;
		
	} else if (!strcmp(s[0], "setworldlocale")) {
	    if (n < 2) goto ARG_ERR;
	    if (!(world_locale = alloc_and_copy(s[1]))) goto CLOSE_RET;
	} else {
	    if (n < 6) goto ARG_ERR;
	    all_size = sizeof(XIMLangDataBase) + strlen(s[0]) + strlen(s[1])
		     + strlen(s[2]) + 3;
	    ...
	    if (!(lnl_p = (XIMLcNameRec *)Malloc(sizeof(XIMLcNameRec) *
						 lc_cnt))) {
		malloc_error("allocation of the initial area");
		goto CLOSE_RET;
	    }
		...
		for (lnl = lc_name_list; lnl; lnl = lnl->next) {
			...
			print_out2(
			"In xim.conf file \"%s\", locale \"%s\" is redeclared.",
				   filenm, ptr);
			goto CLOSE_RET;
		    }
		}
		...
		print_out2(
	    "In xim.conf file \"%s\", \"%s\" is a bad string for CSWIDTH.",
			   filenm, s[3]);
		goto CLOSE_RET;
	    }
	    ...
	    if (!ld->f_table) {
		print_out2(
	    "In xim.conf file \"%s\", \"%s\" is a bad string for functions.",
			   filenm, s[5]);
		goto CLOSE_RET;
	    }
	    if (language_db) {
    ...
    fclose(fp);
    if (!def_locale)
	if (!(def_locale = alloc_and_copy(DEFAULT_LANG))) return(-1); 
    if (!world_locale)
	if (!(world_locale = alloc_and_copy(DEFAULT_WORLD_LOCALE))) return(-1);
    return(0);
ARG_ERR:
    print_out2("In xim.conf file \"%s\", too few arguments at line \"%s\".",
	       filenm, buf);
CLOSE_RET:
    fclose(fp);
    return(-1);
