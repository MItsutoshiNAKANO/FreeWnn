/*
 * $Id: util.c,v 1.1.1.1 2000-01-16 05:07:57 ura Exp $
 */
...
XIMNestLangList lc_list;
{
    ...
    for (; p != NULL; p = p->next) {
	if (!strcmp(p->lc_name, world_locale)) return(True);
    }
    return(False);
}
...
*/
	...
	if (IsPreeditNothing(xc)) {
		...
		reply1.nbytes2 =
			 strlen(root->ximclient->cur_xl->cur_lc->lc_name);
	    }
	} else if (xc->world_on) {
	    reply1.nbytes2 = strlen(world_locale);
...
*/
    ...
    if (tmp_nbytes > 0) {
	if (IsPreeditNothing(xc)) {
	    if (_WriteToClient(root->ximclient->using_language, tmp_nbytes)
		== -1) return;
	} else {
	    if (_WriteToClient(xc->using_language, tmp_nbytes) == -1) return;
	}
    }
    if (tmp_nbytes2 > 0) {
	if (IsPreeditNothing(xc)) {
		...
		if (_WriteToClient(root->ximclient->cur_xl->cur_lc->lc_name,
				   tmp_nbytes2) == -1) return;
	    }
	} else {
	    if (xc->world_on) {
		if (_WriteToClient(world_locale, tmp_nbytes2) == -1)
		...
		if (_WriteToClient(xc->cur_xl->cur_lc->lc_name, tmp_nbytes2)
		    == -1)
		    return;
	    }
	}
...
ChangeIC()
{
	...
	if (change_client(xc, &ic_req, &pre_req, &st_req, p,
			  cur_is_world, pre_font, st_font, &detail) == -1) {
	    reply.state = -1;
	    reply.detail = detail;
	} else {
	    reply.state = 0;
...
register XIMClientRec *xc;
{
	...
	if (!IsPreeditNothing(xc) && c_c->func_stack_count >= 0 &&
	    c_c->command_func_stack[c_c->func_stack_count] != 0){
...
XIMNestLangList *nl;
{
    ...
    *nl = NULL;
    if (lang == NULL || *lang == '\0') return(0);
    if (!(save_d = d = alloc_and_copy(lang))) return(-1);
    for (;;) {
	if (p = index((p = d), ';')) {
	    *p = '\0';
	    next = ++p;
	...
	if (!(alloc = Calloc(1, (sizeof(XIMNestLangRec) + lc_name_len
				 + alias_name_len + 2)))) {
    ...
    Free(save_d);
    if (have_world) {
	if (get_langlist(world_lang, &w_ls) < 0) return(-1);
	if (w_ls) {
	    if (pp) {
		ls->next = w_ls;
	    } else {
