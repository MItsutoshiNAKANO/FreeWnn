/*
 * $Id: wnnrc_op.c,v 1.1.1.1 2000-01-16 05:07:57 ura Exp $
 */
...
int rk_pre_load, root;
{
	...
	(num = sscanf(data,"%s %s %s %s %s %s %s %s",code,
		      s[0],s[1],s[2],s[3],s[4],s[5],s[6])) <= 0) return(0);
	...
	if((fp = fopen(s[0], "r")) != NULL) {
	    ...
	    print_out2("Could not open uumrc \"%s\" included in \"%s\".",
		       s[0], open_uumrc_filenm);
	}
    } else if(strcmp(code , "setrkfile") == 0) {
	if (!(rkfile_defined_by_option && (rk_pre_load || root) &&
	      ld == cur_lang) && (num > 1)) {
	    if(expand_expr(s[0], ld->lang) != 0) {
		err_expand(open_uumrc_filenm, s[0]);
	    }
	    copy++; dst1 = &ld->rkfile_name; src1 = s[0];
	}
    } else if (rk_pre_load == 0) {
	if(strcmp(code , "setuumkey") == 0) {
	    if (num > 1) {
		if(ld->uumkey_name) Free(ld->uumkey_name);
		ld->uumkey_name = NULL;
		if(expand_expr(s[0], ld->lang) != 0) {
		    err_expand(open_uumrc_filenm, s[0]);
		}
		copy++; dst1 = &ld->uumkey_name; src1 = s[0];
	    }
	} else if(strcmp(code , "setconvenv") == 0) {
	    if(num > 1) {
		if (get_new_env(NULL, ld, 0) < 0) goto ERROR_RET;
		if(num > 2) {
		...
		if (num == 2) s0or1 = s[0];
			...
			src2 = s[0];
		    copy++; dst2 = &ld->normal_env->host_name;
		}
		if(expand_expr(s0or1, ld->lang) != 0) {
		    err_expand(open_uumrc_filenm, s0or1);
		}
		copy++; dst1 = &ld->normal_env->envrc_name; src1 = s0or1;
	    }
	} else if(strcmp(code , "setkankanaenv") == 0) {
	    if(num > 1) {
		if (get_new_env(NULL, ld, 1) < 0) goto ERROR_RET;
		if(num > 2) {
		    if(!(strcmp(s[num - 2], "sticky"))) {
		...
		if (num == 2) s0or1 = s[0];
			...
			src2 = s[0];
		    copy++; dst2 = &ld->reverse_env->host_name;
		}
		if(expand_expr(s0or1, ld->lang) != 0) {
		    err_expand(open_uumrc_filenm, s0or1);
		}
		copy++; dst1 = &ld->reverse_env->envrc_name; src1 = s0or1;
	    }
		...
		copy++; dst1 = &ld->jishopath; src1 = s[0];
	    }
		...
		copy++; dst1 = &ld->hindopath; src1 = s[0];
	    }
		...
		copy++; dst1 = &ld->fuzokugopath; src1 = s[0];
	    }
    ...
    if (copy) {
	if (copy > 1) {
	    if (!(*dst2 = alloc_and_copy(src2))) return(-1);
	}
	if (!(*dst1 = alloc_and_copy(src1))) return(-1);
    }
    return(0);
ERROR_RET:
...
{
    if (src->host_name &&
	!(dst->host_name = alloc_and_copy(src->host_name))) return(-1);
...
register XIMLangDataBase *ld;
{
	...
	if (get_new_env(c_c, NULL, 0) < 0) goto ERROR_RET;
	pp = c_c->cur_normal_env;
	if (copy_wnn_env(p, pp) < 0) goto ERROR_RET;
    }
    for (p = ld->reverse_env; p; p = p->next) {
	if (get_new_env(c_c, NULL, 1) < 0) goto ERROR_RET;
...
int rk_pre_load, root;
{
	...
	if((fp = fopen(open_uumrc_filenm, "r")) == NULL) {
	    print_out2(
	"I could not open specified uumrc file \"%s\" for lang \"%s\".",
		       open_uumrc_filenm, ld->lang);
	    err = 1;
	}
    }
    if (fp == NULL) {	/* Open default uumrc */
	if(strcpy(open_uumrc_filenm, LIBDIR),
	   strcat(open_uumrc_filenm, "/"),
	   strcat(open_uumrc_filenm, ld->lang),
	   ...
	   (fp = fopen(open_uumrc_filenm, "r")) != NULL) {
	    ...
	    print_out1("I could not open a default uumrc file \"%s\".",
		       open_uumrc_filenm);
	    return(-1);
	}
    }
    ...
    if (ld->rk_table == NULL) {
	if ((ld->rk_table = romkan_table_init(NULL, ld->rkfile_name, NULL,
					      bytcntfn, NULL, 0))
/*
...
read_default_rk()
{
	...
	for (ld = language_db; ld; ld = ld->next) {
	    if (!strcmp(ld->lang, rr->name)) break;
	}
	if (!ld) {
	    print_out1("In ximrc, I don't support such language \"%s\".",
		       rr->name);
	    return(-1);
	}
