/*
 * $Id: uif.c,v 1.1.1.1 2000-01-16 05:07:57 ura Exp $
 */
...
isconect_jserver()
{
	...
	print_msg_getc(" %s", msg_get(cd, 27, default_message[27], cur_lang->lang),
		       NULL, NULL);
	return (0);
    }
    return(1);
...
kankana_ren_henkan()
{
    if (c_c->cur_reverse_env && reverse_envrcname &&
	*reverse_envrcname != '\0') {
	not_to_han_or_hira();
...
tan_henkan_dai()
{
    if (!c_c->cur_reverse_env) return(0);
    jl_env_set(bun_data_, env_reverse);
...
int in;
{
    ...
    if(!jl_isconnect_e(env_normal) ||
       c_c->cur_reverse_env && env_reverse && !jl_isconnect_e(env_reverse)) {
...
int in;
{
	...
	if ((c < TBL_SIZE) && (quote_flag == 0) &&
	    (c_b->key_table[c] == henkan_off)){
	    break;
	}
