/*
 * $Id: jis_in.c,v 1.1.1.1 2000-01-16 05:07:56 ura Exp $
 */
...
char *lang;
{
	...
	if (multi > 1) {
	    sprintf(jis_title, "%s %d.0x%s",
		    msg_get(cd, MES1_NUM, default_message[MES1_NUM], lang),
		    cs, st);
	} else {
	    sprintf(jis_title, "%s 0x%s",
		    msg_get(cd, MES1_NUM, default_message[MES1_NUM], lang),
		    st);
	}
	jis_ichi_step++;
    }
    if((i = xw_select_one_element(buf_ptr, maxh, 0, jis_title,
...
NEXT:
	...
	if((code = get_jis_ichiran(start_p, c_p-(dot?os:0), in, multi, cs,
				   lang)) == -1){
	    jis_step = 0;
	    goto start;
