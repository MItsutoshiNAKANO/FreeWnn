...
char *displayname;
{
    ...
    if (init_net_area() < 0) return(port);
#ifdef	UNIXCONN
...
close_net()
{
	...
	for (;;) {
	    ...
	    if (accept(accept_blk[UNIX_ACPT].sd, (struct sockaddr *)&addr_un,
		       &addrlen) < 0) break;
	}
	...
	for (;;) {
	    ...
	    if (accept(accept_blk[INET_ACPT].sd, (struct sockaddr *)&addr_in,
		       &addrlen) < 0) break;
	}
...
char *p;
{
    ...
    if (!(s = index(p, ':'))) return(-1);
    if (!(s = index(s, '.'))) return(-1);
    if (!*(++s)) return(-1);
    return(atoi(s));
}
...
wait_for_socket()
{
	...
	if ((n = select(nofile, ready_socks, dummy1_socks, dummy2_socks, 
			(struct timeval *)NULL)) == -1) {
	    if (errno != EINTR) do_end();
	    else {
		bzero(ready_socks, sel_width);
		return(0);
	    }
...
#if defined(XJPLIB) && defined(XJPLIB_DIRECT)
	if (ret = XJp_wait_for_socket()) return(0);
#endif	/* defined(XJPLIB) && defined(XJPLIB_DIRECT) */
	return(0);
    }
}
...
int *p_len, r_len;
{
	...
	if (*p_len <= r_len) {
		...
		for (;r_len > 0; r_len--) {	/* read data in dummy */
		    if (_ReadFromClient(&tmp, 1) == 1) return(-1);
		}
		return(-2);
	    }
	    if (*p) Free(*p);
