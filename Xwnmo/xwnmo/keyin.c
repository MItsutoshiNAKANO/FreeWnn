/*
 * $Id: keyin.c,v 1.1.1.1 2000-01-16 05:07:56 ura Exp $
 */
...
Window set_window;
{
    ...
    if (xim->j_c && (set_window == xim->cur_j_c_root->ichi->w ||
	 set_window == xim->cur_j_c_root->inspect->w)) {
	cur_p = xim->j_c;
	cur_lang = cur_p->cur_xl->lang_db;
	c_c = cur_p->cur_xl->w_c;
	cur_rk = c_c->rk;
