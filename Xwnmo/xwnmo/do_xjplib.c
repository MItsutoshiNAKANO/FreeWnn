/*
 * $Id: do_xjplib.c,v 1.1.1.1 2000-01-16 05:07:55 ura Exp $
 */
...
XJp_init()
{
    ...
    *(p += sizeof(long)) = '\0';
    XChangeProperty(dpy, xim->root_pointer[xim->default_screen]->root_window,
		    xjp_select_id, XA_STRING, 8, PropModeReplace, buffer,
		    p - buffer);
...
XClientMessageEvent *ev;
{
	...
	if ((xjp->mask & XJP_FG) == 0) xjp->fg = XIMCLIENT->pe.fg;
	if ((xjp->mask & XJP_BG) == 0) xjp->bg = XIMCLIENT->pe.bg;
	...
	xc = create_client(&ic_req, &pre_req, &st_req, lc_list, lc_list,
			   0, 0, font, font, xjp->c_data, &detail);
    }
    if (font) Free(font);
...
register XIMClientRec *xc;
{
	...
	if (i->pclient && i->pclient->xim_client == xc) return(1);
    }
    return(0);
}
...
XJp_get_client_cnt()
{
    ...
    for (i = 0, p = xjp_clients; p; p = p->next) i++;
    return(i);
}
#endif
