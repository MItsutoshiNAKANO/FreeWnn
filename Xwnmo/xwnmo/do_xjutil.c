/*
 * $Id: do_xjutil.c,v 1.1.1.1 2000-01-16 05:07:56 ura Exp $
 */
...
char *prop_name;
{
    ...
    if (!(ld = xl->lang_db) || (ld->xjutil_pid)) return(NULL);
    ...
    XChangeProperty(dpy, ROOT_WINDOW, id, XA_STRING, 8, PropModeReplace,
		    (unsigned char *)xj, sizeof(Xjutil_startRec));
	    
    for (i = 0; i < xim->screen_count; i++) {
	XChangeProperty(dpy, ROOT_WINDOW, id, XA_STRING, 8,
			PropModeAppend, (unsigned char *)&cur_p->pe.fg,
			sizeof(unsigned long));
	XChangeProperty(dpy, ROOT_WINDOW, id, XA_STRING, 8,
			PropModeAppend, (unsigned char *)&cur_p->pe.bg,
			sizeof(unsigned long));
	XChangeProperty(dpy, ROOT_WINDOW, id, XA_STRING, 8,
			PropModeAppend,
			(unsigned char *)&cur_p->root_pointer->bw,
			sizeof(unsigned int));
    }
    for (i = 0, we = c_c->normal_env; we; we = we->next, i++) {
	tmp_buf[0] = '\0';
    ...
    XChangeProperty(dpy, ROOT_WINDOW, id, XA_STRING, 8, PropModeAppend, p,
		    strlen((char*)p));
    XSetSelectionOwner(dpy, id,
...
int	id;
{
    ...
    XSendEvent(dpy, cur_p->cur_xl->lang_db->xjutil_id,
...
check_cur_xjutil()
{
    ...
    if (xl->lang_db->xjutil_id == 0 || xl->lang_db->xjutil_use) return(-1);
    xl->working_xjutil = 1;
...
set_uum_env()
{
	...
	jl_env_get(bun_data_) == c_c->cur_reverse_env->env)
...
xw_jishoadd()
{
    if (check_cur_xjutil()) return;
    if (set_uum_env() == 0) return;
