/*
 * $Id: init_w.c,v 1.1.1.1 2000-01-16 05:07:56 ura Exp $
 */
...
register XErrorEvent *ev;
{
	...
	if (ev->resourceid == ld->xjutil_id) {
	    if (ld->xjutil_use) ld->xjutil_use->cur_xl->working_xjutil = 0;
	    break;
	}
    }
#endif	/* USING_XJUTIL */
...
XIMRootRec *root;
{
    ...
    *(p += sizeof(long)) = '\0';
    ...
    XChangeProperty(dpy, root->root_window, xim_id, XA_STRING, 8,
...
XIMRootRec *root;
{
    ...
    if (alloc_for_save_buf(ichi, MAX_ICHI_LINES) < 0) return(-1);
    ...
    ichi->w = XCreateSimpleWindow(dpy, root->root_window, 0, 0,
				  width, height, root->bw, fg, bg);
    ...
    ichi->subtitle = create_box(ichi->w, 0, 0, FontWidth(xl), FontHeight(xl),
				1, fg, bg, fg, 0, '\0');
    ichi->comment = create_box(ichi->w, 0, 0, FontWidth(xl), FontHeight(xl),
				1, fg, bg, fg, 0, '\0');
    if (!ichi->title || !ichi->subtitle || !ichi->comment) return(-1);
	...
	ichi->button[i] = create_box(ichi->w, 0, 0, FontWidth(xl),
				     FontHeight(xl), 2, fg, bg, fg, cursor2, 1);
	if (!ichi->button[i]) return(-1);
    ...
    set_wm_properties(ichi->w, 0, 0, width, height, SouthWestGravity,
		      name, class, True, 0, InputHint, 0, 0);
...
XIMRootRec *root;
{
    ...
    ins->w = XCreateSimpleWindow(dpy, root->root_window, 0, 0,
				 width, height, root->bw, fg, bg);
    ...
    ins->title = create_box(ins->w, 0, 0, FontWidth(xl), FontHeight(xl), 1,
			    fg, bg, fg, 0, '\0');
    if (!ins->title) return(-1);
    ins->title->redraw_cb = set_j_c;
    for (i = 0; i < ins->max_button; i++) {
	ins->button[i] = create_box(ins->w, 0, 0, FontWidth(xl),
				    FontHeight(xl) , 2, fg, bg, fg, cursor2, 1);
	if (!ins->button[i]) return(-1);
	ins->button[i]->redraw_cb = set_j_c;
    }
    ins->button[CANCEL_W]->cb = ins->button[DELETE_W]->cb
			      = ins->button[USE_W]->cb = cur_cl_change3;
    ins->button[CANCEL_W]->do_ret = ins->button[DELETE_W]->do_ret
				  = ins->button[USE_W]->do_ret = True;
    ...
    set_wm_properties(ins->w, 0, 0, width, height, SouthWestGravity,
		      name, class, True, 0, InputHint, 0, 0);
...
XIMClientRec *xc;
{
    ...
    if (!(yes_no->w = XCreateSimpleWindow(dpy, root->root_window,
					  0, 0, width, height, root->bw,
					  fg, bg))) {
    ...
    if (!(yes_no->title = create_box(yes_no->w, 0, 0, width, height, 0, fg, bg,
				     fg, 0, '\0'))) return(-1);
    ...
    set_wm_properties(yes_no->w, 0, 0, width, height, CenterGravity,
		      "yesNo", "YesNo", True, 0, InputHint, 0, 0);
    XFlush(dpy);
    return(0);
}
...
read_wm_id()
{
	...
	for (i = 0; i < xim->screen_count; i++) {
	    ...
	    XChangeProperty(dpy, root->ximclient->w,
			    wm_id, XA_INTEGER, 32, PropModeReplace,
			    (unsigned char*)&wm_id1, sizeof(Atom));
	    XChangeProperty(dpy, root->ximclient->yes_no->w,
			    wm_id, XA_INTEGER, 32, PropModeReplace,
			    (unsigned char*)&wm_id1, sizeof(Atom));
	    XChangeProperty(dpy, root->ichi->w, wm_id, XA_INTEGER,
			    32, PropModeReplace, (unsigned char*)&wm_id1,
			    sizeof(Atom));
	    XChangeProperty(dpy, root->inspect->w, wm_id, XA_INTEGER,
			    32, PropModeReplace, (unsigned char*)&wm_id1,
			    sizeof(Atom));
	}
    } else {
...
XIMClientRec *xc;
{
	...
	icon_mask = XParseGeometry(sub_resource.icon_geometry_res,
				   &icon_x, &icon_y, &icon_width, &icon_height);
    }

    xc->w = XCreateSimpleWindow(dpy, root->root_window,
				ClientX(xc), ClientY(xc),
				ClientWidth(xc), ClientHeight(xc),
				root->bw, xc->pe.fg, xc->pe.bg);
    if (!xc->w) {
...
*/
    ...
    set_wm_properties(xc->w, ClientX(xc), ClientY(xc),
		      ClientWidth(xc), ClientHeight(xc), SouthWestGravity,
		      name, class, False, state, flags, icon_x, icon_y);
...
int screen;
{
    ...
    all_size = size_of_ximclient + size_of_ichiran + size_of_inspect
	     + size_of_yes_no;
    ...
    XtGetSubresources(toplevel, (XtPointer) &sub_resource, sub_name, sub_class,
		      sub_resources, XtNumber(sub_resources), args, 2 );
    ...
    if (root->bw == 0) root->bw = DEFAULT_BORDER_WIDTH;

    /*      Create GC   */
    xgcv.foreground = xc->pe.fg;
    xgcv.background = xc->pe.bg;
    xc->pe.gc = xc->st.gc
      = XCreateGC(dpy, root->root_window, (GCForeground | GCBackground ),&xgcv);
    /*      Create reverse GC   */
    ...
    xc->pe.invertgc = XCreateGC(dpy, root->root_window,
    ...
    if ((xc->have_world = get_langlist(lc_name, &lc_list)) < 0) return(-1);
    if (xc->have_world)
	xc->world_on = 1;
    else
	xc->world_on = 0;
	...
	if(!setlocale(LC_ALL, lc->lang_db->lc_name)) {
	    print_out1("Could not set locale \"%s\" for my internal locale.",
		       lc->lang_db->lc_name);
	...
	if (sub_resource.font_set_res) {
	    if (load_font_set(xl, sub_resource.font_set_res,
			      sub_resource.font_set_res) == -1) {
		print_out2(
	"Could not create any fontset for \"%s\" in locale \"%s\".",
			   sub_resource.font_set_res, lc->lc_name);
		print_out("I will use defualt fontset.");
	    }
	}
	if (xl->pe_fs == NULL) {
	    if ((def_font = get_default_font_name(xl->lang_db->lang)) == NULL) {
	        print_out1("Could not find default fontset for %s",
			   xl->lang_db->lang);
		goto ERROR_OCCUR1;
	    }
	...
	if (i == 0) {
		...
		ClientY(xc) = DisplayHeight(dpy, root->screen)
			    - ClientHeight(xc) - (root->bw * 2);
	    }
	    if (create_xim_window(root, xc) < 0) goto ERROR_OCCUR2;
	...
	if (new_client(xc, xl, (screen == 0)?True:False) == -1) {
	    print_out1("I give up creating of environment for locale \"%s\".",
		      xl->lang_db->lc_name);
	    goto ERROR_OCCUR3;
	}
...
ERROR_OCCUR1:
    ...
    if (create_ichi(root) < 0 || create_inspect(root) < 0) return(-1);
...
char *lc_name;
{
    ...
    all_size = size_of_xim + size_of_root_pointer
	     +(size_of_root * screen_count);
    if ((p = (char *)Malloc(all_size)) == NULL) {
	malloc_error("allocation of the initial area");
	return(-1);
    }
	...
	*(xim->supported_language + (lang_len - 1)) = '\0';
    ...
    XSetWMClientMachine(dpy,
			xim->root_pointer[xim->default_screen]->ximclient->w,
			&machineName);
#endif	/* X11R3 */
    XChangeProperty (dpy, xim->root_pointer[xim->default_screen]->ximclient->w,
		     XA_WM_COMMAND, XA_STRING, 8, PropModeReplace,
		     (unsigned char *)arg_string, strlen(arg_string));
    return(0);
}
