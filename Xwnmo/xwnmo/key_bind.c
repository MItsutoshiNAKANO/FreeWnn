/*
 * $Id: key_bind.c,v 1.1.1.1 2000-01-16 05:07:56 ura Exp $
 */
...
char *lang;
{
    ...
    if (!(p = (struct kansuu **)Calloc(ONE_CHAR_TBL * ('z' - 'a' + 1),
				       sizeof(struct kansuu *)))) {
	...
	*p = &kansuu_hyo[k];
	...
	if (strcmp(func_name, "include")== 0) {
	    ...
	    for (k = 0; k < ONE_CHAR_TBL && *p; k++, p++) {
		if (strcmp(func_name, (*p)->kansuumei)== 0) { /*found!*/
		    if ((*p)->key_binded) remove_key_bind(*p);
		    (*p)->key_binded = (char)1;
...
char *lang;
{
    FILE *fp = NULL, *fopen(); char file_name[PATHNAMELEN];
    int j,k;
    extern int fclose();
	...
	if (fp = fopen(file_name , "r")) fclose(fp);
    }
    if (!fp) {
	if (strcpy(file_name , LIBDIR),
	    strcat(file_name, "/"),
