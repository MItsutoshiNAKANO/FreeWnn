/*
 * $Id: prologue.c,v 1.1.1.1 2000-01-16 05:07:56 ura Exp $
 */
...
XIMLangDataBase *db;
{
	...
	if (c_c->normal_env == NULL) {
	    if (get_new_env(c_c, NULL, 0) < 0) return(-1);
	}
	c_c->use_server = 0;
	c_c->cur_normal_env = c_c->normal_env;
	uumrc_name[0] = '\0';
	if (!(bun_data_ = local_jl_open_lang())) return(-1); 
	return(0);
    } else {
	c_c->use_server = 1;
    }

    if (c_c->cur_normal_env) save_cur_normal_env = c_c->cur_normal_env;
	...
	if (bun_data_ == NULL) {
	    bun_data_ = jl_open_lang(environment, servername, db->lang,
				     uumrc_name, yes_or_no_lock,
	    ...
	    env_normal = jl_connect_lang(environment, servername,
					  db->lang, uumrc_name, yes_or_no_lock,
					  print_msg_wait, WNN_TIMEOUT);
	}
	if (env_normal) {
	    if(normal_sticky)
	...
	if (env_normal) jl_env_set(bun_data_, env_normal);
    }
    if (c_c->normal_env == NULL) {
	if (get_new_env(c_c, NULL, 0) < 0) return(-1);
	c_c->use_server = 0;
    ...
    if (c_c->cur_reverse_env) save_cur_reverse_env = c_c->cur_reverse_env;
    ...
    if (save_cur_reverse_env) c_c->cur_reverse_env = save_cur_reverse_env;
#ifdef  USING_XJUTIL
...
init_xcvtkey()
{
	...
	if ((ret = cvt_meta_and_fun_setup(cvt_fun_file, cvt_key_tbl,
					  NULL)) == -1) {
	...
	if ((ret = cvt_meta_and_fun_setup(cvt_meta_file, cvt_key_tbl,
					  "Meta")) == -1) {
	...
	if (cvt_key_setup(cvt_key_file) == -1) {
	    print_out1("I can't open cvt_key file \"%s\", use default file.",
		      cvt_key_file);
	    cvt_key_file = NULL;
	}
    }
	...
	if (!(cvt_key_file = alloc_and_copy(tmp_file))) return(-1);
	if (cvt_key_setup(cvt_key_file) == -1) {
	    print_out1(
  "I can't open default cvt_key file \"%s\".\n I don't convert function keys.",
		       cvt_key_file);
	    Free(cvt_key_file);
	    cvt_key_file = NULL;
	}
