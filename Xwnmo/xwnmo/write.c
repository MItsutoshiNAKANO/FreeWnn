/*
 * $Id: write.c,v 1.1.1.1 2000-01-16 05:07:57 ura Exp $
 */
...
register int ct_len;
{
    ...
    if (_WriteToClient(&reply, sz_ximReturnReply) == -1) return(-1);
    if (ct_len > 0) {
	if (_WriteToClient(ct, ct_len) == -1) return(-1);
    }
    return(0);
}
...
char str;
{
    ...
    if (_WriteToClient(&reply, sz_ximReturnReply) == -1) return(-1);
    if (tmp_length > 0) {
	if (_WriteToClient(buf, tmp_length) == -1) return(-1);
    }
    return(0);
}
...
register int	size;
{
	...
	if (!((w_c = *w_buf) & 0xff80)) {
	    if (conv_len) {
		while (1) {
		    send_len = w_char_to_ct(xl->xlc, start, ct_buf, conv_len,
					    ct_buf_max);
		    if (send_len < -1) {
			break;
		    } else if (send_len == -1) {		    
			if (realloc_ct_buf() < 0) break;
		    } else if (send_len > 0) {
#ifndef X11R5
#ifdef BC_X11R5
			if (IS_XIMR5_CLIENT())
