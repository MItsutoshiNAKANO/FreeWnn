...
make_world_lang()
{
    ...
    if (!(world_lang = Malloc(len))) return(-1);
    world_lang[0] = '\0';
    for (ld = language_db; ld; ld = ld->next) {
...
/*    extern char *index(); */
	...
	if((name = getenv(WNN_USERNAME_ENV)) != NULL){
	    if (!(root_username = alloc_and_copy(name))) exit(1);
	} else {
	...
	if (db == NULL || db->lang == NULL) {
	    print_out1("Sorry, langName \"%s\" is not my supported language.",
		       def_lang);
	    print_out("I refer to the system locale.");
	} else if (lc_name == NULL) {
	    print_out2(
		"Sorry, can not set locale \"%s\" related to langName \"%s\".",
		       db->lc_name, def_lang);
	    print_out("I refer to the system locale.");
	}
    }
	...
	if (!strcmp(lc_name, world_locale)) {
		...
		if (!strcmp(lnl->lc_name, lc_name)) break;
	    }
	}
	if (!lnl) {
	    print_out1("Sorry, locale \"%s\" is not my supported locale.",
		       lc_name);
	    print_out("I will abort.");
	    exit(1);
	}
	...
	if(name = getenv(server_env)) {
	    if (!(root_def_servername = root_def_reverse_servername
				      = alloc_and_copy(name))) exit(1);
	} else {
	...
	init_xcvtkey() < 0 ||
	alloc_all_buf() < 0 ||
	create_xim(lc_name_buf) < 0) /* create X window environment */
	exit(1);
