/*
 * $Id: ximdispt.c,v 1.1.1.1 2000-01-16 05:07:57 ura Exp $
 */
    ...
    int		num;
{
    ...
    if ((ret = _WriteToClient(p, num)) >= 0) {
    	if (_Send_Flush() == 0) return ret;	/* success */
    }
    return ret;
}
    ...
    ximICAttributesReq	*ic_attr_req;
{
    ...
    if (size != (sizeof(CARD16) << 2)) return XIM_BadAlloc;
    ...
    ximICAttributesReq	*ic_attr_req;
{
    ...
    if (size != (sizeof(CARD16) + sizeof(CARD16))) return XIM_BadAlloc;
    ...
    char		**base_font_name;
{
    ...
    if (len > size) return XIM_BadAlloc;
    ...
    char		**st_font;
{
	...
	len = (int)GET_CARD16(buf_16[1]);
   	...
   	size -= tmpl;
	    ...
	    mask = 0;
	    ret = DecodeICNestedAttributes(list, len,
			 &mask, pre_req, pre_font);
	    ic_req->mask |= mask;
	    break;
    ...
    XIMClientRec	*xc;
{
    ...
    /* parse IC attributes */
    ret = DecodeICAttributes(list, size, 
			&ic_req, &pre_req, &st_req, &pre_font, &st_font);
    if (ret) return ret;

    /* refer to util: ChangeIC() */

    ret = change_client(xc, &ic_req, &pre_req, &st_req, p,
		      cur_is_world, pre_font, st_font, &detail);
    if (pre_font) XFree(pre_font);
    if (st_font) XFree(st_font);
    if (p) free_langlist(p);
    if (ret == -1) return ConvErrorCode(detail);
    return 0;
}
 ...
 */
static int
EncodeICNestedAttributes(attr_id_list, size, xc, ic_attr, 
		value, size_return, num_return)
    CARD8		*attr_id_list;
    int			size;
    XIMClientRec	*xc;
    ...
    int			*size_return;
{
	...
	case ICFilterEvents:
	    ...
	    break;
	case ICPreeditAttributes:
	    ret = EncodeICNestedAttributes((attr_id_list + sizeof(CARD16)),
			(size - sizeof(CARD16)),
			xc, &(xc->pe), data, &len, &num);	
	    if (!ret) {
    ...
    XICID		*ic_id_return;
{
	...
	(!(ic_req.mask & (1 << ICFontSet)) || !pre_font || !*pre_font))
	return XIM_BadName;
	...
	if (cur_l && *cur_l) {
	    ...
	    if (have_world == 2) cur_is_world++;
	    cur_lc = lc_list;
	}
    }
    xc = create_client(&ic_req, &pre_req, &st_req, lc_list, cur_lc,
		       have_world, cur_is_world, pre_font, st_font,
		       c_data, &detail);
    if (pre_font) XFree(pre_font);
    if (st_font) XFree(st_font);
    ...
    register int	size;
{
    ...
    if (size == 0) index = XIM_Default_Encoding_IDX;	/* use default */
    return index;
}
    ...
    XimRequestPacket	*req;
{
    ...
    if (_ReadFromClient((char *)req, XIM_HEADER_SIZE) == -1) return -1;
    ...
    XimRequestPacket	*req;
{
    ...
    if (major_version != PROTOCOLMAJORVERSION ||
	minor_version != PROTOCOLMINORVERSION)
	return XIM_BadProtocol;
    ...
    int			*pre_len_returned;
{
    ...
    *pre_len_returned = 0;
    ...
    if (wc_len > 0) {
        while (1) {
            send_len = wchar_to_ct(cur_p->cur_xl->xlc, JW_buf, ct_buf, wc_len,
                               ct_buf_max);
            if (send_len < -1) {
                return;
            } else if (send_len == -1) {
                if (realloc_ct_buf() < 0) return;
            } else {
                break;
            }
        }
    ...
    int			*pre_len_returned;
{
	...
	cur_p = save_cur_p;
	cur_x = save_cur_x;
	if (ret == -1) return XIM_BadProtocol;
    }
    return 0;
}
    ...
    XimRequestPacket	*req;
{
     ...
     */
    if (ret) return ((flag & XimSYNCHRONUS)? ret: 0);
    ...
    int		feedback_num;
{
    ...
    if (str_len > 0) {
        ...
        buf_16 = (CARD16 *)((CARD8 *)buf_16 + sizeof(CARD16) +
                                 PAD(sizeof(CARD16) + str_len));
    } else {
        status |= XIMNoString;
    }
    if (feedback_num > 0) {
 ...
 */
static int
SendPreeditDraw(im_id, ic_id, caret, chg_first, chg_len,
		str, str_len, feedback, feedback_num)
    XIMID	im_id;
    XICID	ic_id;
    INT32	caret;
    INT32	chg_first;
 ...
 */
static int
SendStatusDraw(im_id, ic_id, st_type, str, str_len, 
		feedback, feedback_num, pixmap)
    XIMID	im_id;
    XICID	ic_id;
    CARD32	st_type;
    char	*str;
    ...
    Atom	imserver;
{
    ...
    (void) XGetWindowProperty(disp, win, server_atom, 0L, 1000000L,
	...
	(void) memcpy((char *)new_list, (char *)prop_return,
					nitems * sizeof(Atom));
     ...
     */
    XChangeProperty(disp, win, server_atom, XA_ATOM, 32,
		PropModeReplace, (unsigned char *)new_list, (nitems + 1));
    if (nitems > 0) {
...
#endif /* NeedFunctionPrototypes */
{
    ...
    if (!(locale_prop = (unsigned char *)Malloc(len + 
			strlen(locale_supported) + 1))) {
    ...
    (void) sprintf(atom_name, "%stcp/%s:%u", XIM_TRANSPORT_CATEGORY, 
			hostname, port);
    len = strlen(atom_name);
...
#endif /* NeedFunctionPrototypes */
{
    return SendCommit(CUR_IM_ID(), CUR_IC_ID(),
		XimLookupChars, (KeySym)0, ct, ct_len);
}
...
#endif /* NeedFunctionPrototypes */
{
    if (keysym < 0x20) {
        keysym |= 0xff00;
        return SendCommit(CUR_IM_ID(), CUR_IC_ID(),
			 XimLookupKeySym, keysym, (char *)NULL, 0);
    } else {
	str &= 0xff;
        return SendCommit(CUR_IM_ID(), CUR_IC_ID(),
			XimLookupBoth, keysym, &str, 1);
    }
}
...
#endif /* NeedFunctionPrototypes */
{
    ...
    if (str) {
	while (1) {
	    send_len = wchar_to_ct(cur_p->cur_xl->xlc, str, ct_buf, len,
				   ct_buf_max);
	    if (send_len < -1) {
		return;
	    } else if (send_len == -1) {
    ...
    (void) SendPreeditDraw(CUR_IM_ID(), CUR_IC_ID(), caret, first, chg_len,
		ct_buf, send_len, (XIMFeedback *)&xim_fb, 1);
}
...
#endif /* NeedFunctionPrototypes */
{
    ...
    (void) SendPreeditCaret(CUR_IM_ID(), CUR_IC_ID(), pos, 
		XIMAbsolutePosition, style);
}
...
#endif /* NeedFunctionPrototypes */
{
    ...
    (void) SendStatusDraw(CUR_IM_ID(), CUR_IC_ID(), XIMTextType,
		ct_buf, send_len, (XIMFeedback *)NULL, 0, (PIXMAP)0);
}
