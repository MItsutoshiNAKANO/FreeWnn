/*
 * $Id: ichiran.c,v 1.1.1.1 2000-01-16 05:07:56 ura Exp $
 */
...
int y;
{
    ...
    if (y < 0 || y > ICHI->item_height) return(-1);
...
#endif /* !XJUTIL */
    ichi = ICHI;
    if ((tmp = p/ichi->max_line + (p % ichi->max_line ? 1 : 0))
	!= ichi->cur_page) {
	ichi->cur_page = tmp;
	move_ichi(ichi);
    }
...
void
set_wm_properties(window, x, y, width, height, gravity, name, class_name,
		  input, state, flags, icon_x, icon_y)
Window window;
int x, y;
unsigned int width, height;
int gravity;
...
int icon_x, icon_y;
{
    ...
    if (name) {
	XSetWMProperties(dpy, window, &winName, &iconName, NULL, 0,
...
#else	/* X11R3 */
    if (name) {
	XSetStandardProperties(dpy, window, winName, iconName, None, NULL, 0,
			       &hints);
	XSetWMHints(dpy, window, &wm_hints);
...
register int next;
{
	...
	if (start_end >= ichi->kosuu) start_end = ichi->kosuu - 1;
    }
...
int
init_ichiran(info, kosuu, init, title, subtitle, exec_title,
	     comment, nyuu_col, mode)
unsigned char **info;
int kosuu, init;
unsigned char *title;
...
int nyuu_col, mode;
{
	...
	if (mode & (JIKOUHO | JIS_IN | KUTEN_IN)) {
		...
		(unsigned char *)msg_get(cd, NEXT_NUM,
					 default_message[NEXT_NUM], lang);
	    ichi->button[BACK_W]->string =
		(unsigned char *)msg_get(cd, BACK_NUM,
					 default_message[BACK_NUM], lang);
#ifndef XJUTIL
	    ichi->button[NEXT_W]->cb_data = ichi->button[BACK_W]->cb_data
					  = (int *)cur_x;
#endif	/* !XJUTIL */
	    def_maxlen = columnlen(title)
		       + columnlen(ichi->button[CANCEL_W]->string)
		       + columnlen(ichi->button[MOUSE_W]->string)
		       + columnlen(ichi->button[NEXT_W]->string)
		       + columnlen(ichi->button[BACK_W]->string)
		       + SPACING * 5 + 1;
	...
	if (ichi->kosuu_all > MAX_PAGE_LINES) {
	    ichi->page = ichi->kosuu_all / MAX_PAGE_LINES
		       + (ichi->kosuu_all % MAX_PAGE_LINES ? 1 : 0);
	    ...
	    if (mode & SENTAKU) {
		def_maxlen += columnlen(ichi->button[NEXT_W]->string)
			   + columnlen(ichi->button[BACK_W]->string)
			   + SPACING * 3;
	    }
	} else {
	    ...
	    height = ichi->kosuu_all * (FONTHEIGHT + IN_BORDER)
		   + 2 * FONTHEIGHT;
	    ichi->page_height = 0;
	}
	if (comment) {
	...
	for (cnt = 1; cnt < ichi->kosuu_all; cnt++) {
	    if ((tmp = columnlen(ichi->buf[cnt])) > maxlen) maxlen = tmp + 2;
	}
	if (maxlen < def_maxlen) maxlen = def_maxlen;
    } else if (mode & NYUURYOKU) {
	def_maxlen = columnlen(title) + columnlen(exec_title)
		   + columnlen(ichi->button[CANCEL_W]->string)
		   + SPACING * 3 + 1;
    ...
    XQueryPointer(dpy, ROOT_WINDOW, &r, &child,
    ...
    changecolor_box(ichi->button[CANCEL_W], FOREGROUND, BACKGROUND, FOREGROUND,
		    BoxAll);
    if (!(mode & NYUURYOKU)) {
	if(comment) {
	    ...
	    changecolor_box(ichi->comment, FOREGROUND, BACKGROUND, FOREGROUND,
			    BoxAll);
	    map_box(ichi->comment);
	} else {
	    y_cood = FONTHEIGHT * 2;
	}
	XMoveResizeWindow(dpy, ichi->w0, -1, y_cood,
			  FONTWIDTH * ichi->max_columns,
			  ichi->max_line * (FONTHEIGHT + IN_BORDER));
	XMoveResizeWindow(dpy, ichi->w1, 0, 0,
			  FONTWIDTH * ichi->max_columns,
			  ichi->kosuu_all * (FONTHEIGHT + IN_BORDER));
#ifndef XJUTIL
	...
	moveresize_box(ichi->button[MOUSE_W],
		       (FONTWIDTH * (len_t += SPACING)),
		       (FONTHEIGHT/2 - IN_BORDER),
		       (columnlen(ichi->button[MOUSE_W]->string) * FONTWIDTH
			+ IN_BORDER * 2), 
		       (FONTHEIGHT + IN_BORDER * 2));
	len_t += columnlen(ichi->button[MOUSE_W]->string);
	changecolor_box(ichi->button[MOUSE_W], FOREGROUND, BACKGROUND,
			FOREGROUND, BoxAll);
    }

    set_wm_properties(ichi->w, x_cood, y_cood, width, height, SouthWestGravity,
		      NULL, NULL, 0, 0, 0, 0, 0);
    ...
    XFlush(dpy);
    if (mode & NYUURYOKU) {
	moveresize_box(ichi->subtitle, FONTWIDTH,
		       (FONTHEIGHT * 2 + FONTHEIGHT/2 - IN_BORDER),
		       (columnlen(subtitle) * FONTWIDTH + IN_BORDER * 2),
		       (FONTHEIGHT + IN_BORDER * 2));
	changecolor_box(ichi->subtitle, FOREGROUND, BACKGROUND, FOREGROUND,
			BoxAll);
	map_box(ichi->subtitle);
	XMoveResizeWindow(dpy, ichi->nyuu_w,
			  FONTWIDTH * (columnlen(subtitle) + 3),
			  FONTHEIGHT * 2 + FONTHEIGHT/2,
			  nyuu_col * FONTWIDTH, FONTHEIGHT);
#ifndef XJUTIL
	...
	moveresize_box(ichi->button[EXEC_W],
		       (FONTWIDTH * (len_t += SPACING)),
		       (FONTHEIGHT/2 - IN_BORDER),
		       (columnlen(exec_title) * FONTWIDTH + IN_BORDER * 2),
		       (FONTHEIGHT + IN_BORDER * 2));
	changecolor_box(ichi->button[EXEC_W], FOREGROUND, BACKGROUND,
			FOREGROUND, BoxAll);
	...
	if (mode & (JIKOUHO | JIS_IN | KUTEN_IN)) {
		...
		ichi->button[MOUSE_W]->string =
			(unsigned char *)msg_get(cd, MOUSE_NUM + 1,
						 default_message[MOUSE_NUM + 1],
						 lang);
		...
		ichi->cur_page =
			((jikouho_ichi[init].line - 1) / ichi->max_line) + 1;
		move_ichi(ichi);
	    }
	} else if (mode & SENTAKU) {
	...
	if(ichi->page > 1) {
	    ...
	    changecolor_box(ichi->button[NEXT_W], FOREGROUND, BACKGROUND,
			    FOREGROUND, BoxAll);
	    changecolor_box(ichi->button[BACK_W], FOREGROUND, BACKGROUND,
			    FOREGROUND, BoxAll);
...
end_ichiran()
{
    ...
    if (ichi->mode & NYUURYOKU) XUnmapWindow(dpy, ichi->nyuu_w);
...
Window w;
{
	...
	if (!xc->cur_xl) continue;
	for (i = 0; i < xim->screen_count; i++) {
	    if (xc == xim->root_pointer[i]->ximclient) break;
	}
	if (i < xim->screen_count) continue;
	if (IsPreeditPosition(xc)) {
	    if (w == xc->cur_xl->wp[0] || w == xc->cur_xl->wp[1] ||
		w == xc->cur_xl->wp[2] || w == xc->cur_xl->wn[0] ||
		w == xc->cur_xl->wn[1] || w == xc->cur_xl->wn[2]) {
		match = 1;
	    }
	} else if (IsPreeditArea(xc)) {
	    if (w == xc->cur_xl->wp[0]) match = 1;
	}
	if (IsStatusArea(xc)) {
	    if (w == xc->cur_xl->ws) match = 1;
	}
	if (match) {
...
unsigned char *msg;
{
    ...
    ins->button[CANCEL_W]->cb_data = ins->button[DELETE_W]->cb_data =
    ins->button[USE_W]->cb_data = ins->button[NEXT_W]->cb_data =
    ins->button[BACK_W]->cb_data = (int *)cur_x;
    moveresize_box(ins->title, FONTWIDTH, (FONTHEIGHT/2 - IN_BORDER),
		   (FONTWIDTH * (len_t = columnlen(ins->title->string))
		    +IN_BORDER * 2),
		   (FONTHEIGHT + IN_BORDER * 2));
    len_t += 1;
    for (i = 0; i < ins->max_button; i++) {
	moveresize_box(ins->button[i], (FONTWIDTH * (len_t += SPACING)),
		       (FONTHEIGHT/2 - IN_BORDER),
		       (columnlen(ins->button[i]->string) * FONTWIDTH
		        + IN_BORDER * 2), (FONTHEIGHT + IN_BORDER * 2));
    ...
    set_wm_properties(ins->w, x_cood, y_cood, width, height, CenterGravity,
		      NULL, NULL, 0, 0, 0, 0, 0);
    ...
    for (i = 0; i < ins->max_button; i++) {
	changecolor_box(ins->button[i], FOREGROUND, BACKGROUND, FOREGROUND,
			BoxAll);
    }
...
register unsigned char *s;
{
	...
	XResizeWindow(dpy, ins->w, (width = FONTWIDTH * len),
		      (height = FONTHEIGHT * 3 + IN_BORDER * 2));
	XResizeWindow(dpy, ins->w1, FONTWIDTH * len,
		      FONTHEIGHT + IN_BORDER * 2);
	ins->max_len = len;
...
int d;
{
	...
	item + (ichi->cur_page - 1) * ichi->max_line < ichi->kosuu) {
	if (ichi->hilited_item != -1)
	    xw_fillrec();
...
int line;
{
    ...
    if (!(p = Malloc((unsigned)(line * (ichi->max_columns * sizeof(char) +
					sizeof(char *)))))) {
	malloc_error("allocation of ichiran data");
	return(-1);
    }
    if (ichi->save_buf) Free(ichi->save_buf);
...
redraw_ichi_w()
{
    ...
    XClearWindow(dpy, ichi->w1);
    XFlush(dpy);
    for (line = ichi->start_line;
	 line <= ichi->end_line && line < ichi->kosuu_all; line++) {
...
xw_next_select()
{
...
#endif /* !XJUTIL */
    ichi = ICHI;
    if (ichi->lock) return(0);
    cur_l = jikouho_ichi[ichi->hilited_ji_item].line;

    if (ichi->mode & (JIKOUHO | JIS_IN | KUTEN_IN)) {
	if (ichi->max_line == 1) return(0);
	if (ichi->hilited_ji_item != -1)
	    xw_ji_fillrec();
	...
	if ((ichi->hilited_item == ichi->end_line) ||
...
int flg;
{
	...
	if (flg == YesMessage) {
		...
		= (unsigned char *)msg_get(cd, OK_NUM, default_message[OK_NUM],
					   lang);
	} else if (flg == YesNoMessage) {
	    yes_no->button[0]->string
		= (unsigned char *)msg_get(cd, YES_NUM,
					   default_message[YES_NUM], lang);
	    yes_no->button[1]->string
		= (unsigned char *)msg_get(cd, NO_NUM, default_message[NO_NUM],
					   lang);
	}
#ifndef XJUTIL
	...
	moveresize_box(yes_no->button[0], (FONTWIDTH * 2),
		       (FONTHEIGHT * 2 + FONTHEIGHT/2 - IN_BORDER),
		       ((len = columnlen(yes_no->button[0]->string))
			* FONTWIDTH + IN_BORDER * 2),
		       (FONTHEIGHT + IN_BORDER * 2));
	changecolor_box(yes_no->button[0], FOREGROUND, BACKGROUND, FOREGROUND,
			BoxAll);
	...
	if (flg == YesNoMessage) {
	    moveresize_box(yes_no->button[1], (FONTWIDTH * (6 + len)),
			   (FONTHEIGHT * 2 + FONTHEIGHT/2 - IN_BORDER),
			   (FONTWIDTH * columnlen(yes_no->button[1]->string)
			    + IN_BORDER * 2),
			   (FONTHEIGHT + IN_BORDER * 2));
	    changecolor_box(yes_no->button[1], FOREGROUND, BACKGROUND,
			    FOREGROUND, BoxAll);
    ...
    if (flg != MessageOnly) {
	XQueryPointer(dpy, ROOT_WINDOW, &r, &child, &yes_no->x,
    ...
    set_wm_properties(yes_no->w, x_cood, y_cood, width, height, CenterGravity,
		      NULL, NULL, 0, 0, 0, 0, 0);
...
XButtonEvent *event;
{
	...
	if (ichi->hilited_item != -1) xw_fillrec();
	if ((ichi->hilited_item = item) != -1) xw_fillrec();
    }
    XFlush(dpy);
    SEL_RET = item;
}
...
XButtonEvent *event;
{
	...
	!= ichi->hilited_ji_item) {
	if (ichi->hilited_ji_item != -1) xw_ji_fillrec();
	if ((ichi->hilited_ji_item = item) != -1) xw_ji_fillrec();
    }
    XFlush(dpy);
    SEL_RET = item;
}
...
xw_mouse_select()
{
	...
	XDefineCursor(dpy, ichi->w1, XCreateFontCursor(dpy,52));
	ichi->button[MOUSE_W]->string = (unsigned char *)msg_get(cd, MOUSE_NUM,
					default_message[MOUSE_NUM], lang);
	ichi->select_mode = 1;
    } else {
	XSelectInput(dpy, ichi->w1, ExposureMask);
	XUndefineCursor(dpy, ichi->w1);
    ...
    JW3Mputc(ichi->button[MOUSE_W]->string,
	     ichi->button[MOUSE_W]->window, 0, REV_FLAG, IN_BORDER);
    XFlush(dpy);
    return(0);
}
...
char *string;
{
	...
	if ((xim->sel_ret == 1) ||
	    (xim->sel_ret == -1 &&
	     (c1 == NEWLINE || c1 == CR || c1 == 'y' || c1 == 'Y'))) {
	    end_yes_or_no();
	    xim->sel_ret = -1;
	    return(1);
	} else if ((xim->sel_ret == 0) ||
	    (xim->sel_ret == -1 && (c1 == 'n' || c1 == 'N'))) {
	    end_yes_or_no();
	    xim->sel_ret = -1;
	    return(0);
	} else {
...
int in;
{
	...
	if ((xim->sel_ret == 1) ||
	    (xim->sel_ret == -1 &&
	     (in == NEWLINE || in == CR || in == 'y' || in == 'Y'))) {
	    ret = 1;
	} else if ((xim->sel_ret == 0) ||
	    (xim->sel_ret == -1 && (in == 'n' || in == 'N'))) {
	    ret = 0;
	} else {
	    xim->sel_ret = -1;
	    return(BUFFER_IN_CONT);
...
char *p;
{
    ...
    JW3Mputc(ichi->buf[k], ichi->w1, k,
	     ((k == ichi->hilited_item)? REV_FLAG:0), IN_BORDER);
}
