/*
 * $Id: do_xjpdrct.c,v 1.1.1.1 2000-01-16 05:07:55 ura Exp $
 */
...
XJp_close_net()
{
	...
	for (;;) {
	    ...
	    if (accept(accept_blk[XJP_INET_ACPT].sd, (struct sockaddr *)&addr_in, &addrlen) < 0) break;
	}
...
char *string;
{
    ...
    if (_WriteToClient(&tmp_short, sizeof(short)) == -1) return(-1);
    tmp_short = (unsigned short)htons(detail);
    if (_WriteToClient(&tmp_short, sizeof(short)) == -1) return(-1);

    if (open == 0 && state == (short)0) {
	tmp_long = (unsigned long)htonl(keysym);
	if (_WriteToClient(&tmp_long, sizeof(KeySym)) == -1) return(-1);
	if (detail > (short)0) {
	    if (_WriteToClient(string, (int)detail) == -1) return(-1);
	}
	need_write = 0;
    }
...
int fd;
{
    ...
    if ((xc = create_client(&ic_req, &pre_req, &st_req, NULL, NULL,
			    0, 0, NULL, NULL, NULL, &detail)) == NULL){
	Free((char*)xjp);
	XJp_return_sock((short)-1, (short)7, 1, (KeySym)0, NULL);
	return(-1);
    }
