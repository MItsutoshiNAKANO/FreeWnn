/*
 * $Id: xlc_util.c,v 1.1.1.1 2000-01-16 05:07:57 ura Exp $
 */
    ...
    int                *scanned_len;
{
    ...
    *mb_bytes = ret;
    if (scanned_len) *scanned_len = wc_len - from_left;
    return ret;
    ...
    int             *state;
{
    ...
    *wc_len = ret;
    if (scanned_bytes) *scanned_bytes = mb_bytes - from_left;
    return 0;
}
...
int alloc_all_buf()
{
    ...
    char *w;
#endif /* !X_WCHAR */
    if (((wc = Malloc(sizeof(wchar) * BUF_ALLOC_SIZE)) == NULL) ||
	((ct = Malloc(sizeof(unsigned char) * BUF_ALLOC_SIZE)) == NULL) ||
	((c = Malloc(sizeof(unsigned char) * BUF_ALLOC_SIZE)) == NULL)
#ifndef X_WCHAR
...
{
    register char *p;
    if ((p = Malloc(sizeof(unsigned char) * (ct_buf_max + BUF_ALLOC_SIZE)))
	== NULL) {
	malloc_error("re-allocation of temporary buffer");
	return(-1);
    }
...
int realloc_c_buf()
{
    ...
    if ((p = Malloc(sizeof(unsigned char) * (c_buf_max + BUF_ALLOC_SIZE)))
	== NULL) {
	malloc_error("re-allocation of temporary buffer");
	return(-1);
    }
...
int ct_len;
{
    ...
    ret = _XConvertMBToCT(xlc, c_buf, ret, ct, &ct_str_len, &scanned_byte,
			  NULL);
    if (ret < 0) {
	if (ret == BadBuffer) {
	    return(-1);
...
int text_len;
{
	...
	ret = _XConvertWCToMB(
			      ...
			      cur_p->cur_xl->xlc,
#endif
			      text, text_len, c_buf, &mb_buf_len,
			      &scanned_byte, NULL);
	if (ret < 0) {
	    if (ret == BadBuffer) {
		if (realloc_c_buf() < 0) return;
	    } else {
		return;
	    }
	} else {
