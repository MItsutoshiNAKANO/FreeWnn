...
XClientMessageEvent *ev;
{
	...
	do_end();
    } else if (xim->j_c && (ev->window == xim->cur_j_c_root->ichi->w ||
	       ev->window == xim->cur_j_c_root->inspect->w)) {
	xim->sel_ret = -2;
...
register XExposeEvent *event;
{
	...
	if (window == xim->cur_j_c_root->ichi->w1) {
	    ...
	    JW3Mputc(xim->cur_j_c_root->ichi->nyuu,
		     xim->cur_j_c_root->ichi->nyuu_w, 0, 1, 0);
	    return;
	}
	if (window == xim->cur_j_c_root->inspect->w1) {
	    JW3Mputc(xim->cur_j_c_root->inspect->msg,
		     xim->cur_j_c_root->inspect->w1, 0, 0, IN_BORDER);
	    return;
	}
    }
    return;
...
XButtonEvent *event;
{
    ...
    xim->sel_ret = -1;
    for (xc = ximclient_list; xc != NULL; xc = xc->next) {
	if (IsPreeditNothing(xc)) continue;
...
#ifdef	XJPLIB
    if (XJp_xw_destroy(event)) return;
#endif	/* XJPLIB */
#ifdef	USING_XJUTIL
    if (xjutil_destroy(event)) return;
#endif	/* USING_XJUTIL */
    for (xc = ximclient_list; xc != NULL; xc = xc->next) {
...
char el;
{
	...
	if (window == xim->cur_j_c_root->ichi->w1) {
		...
		if (xim->cur_j_c_root->ichi->mode == SENTAKU) xw_mouseleave();
	    }
	    return;
	}
    }
...
int force;
{
    ...
    for (i = 0, xc = ximclient_list; xc; xc = xc->next) i++;
#ifdef	XJPLIB
	...
	XGetWindowProperty(dpy,
			   xim->root_pointer[xim->default_screen]->root_window,
			   prop_id, 0L, 1000000L, False, XA_STRING,
			   &actual_type, &actual_format, &nitems, &leftover,
			   &data);
	if (nitems > 0 && data && *data &&
	    !strncmp(root_username, (char *)data, nitems)) {
	    if (force || i == 1) {
		ok = 1;
	...
	switch (event.type) {
	 ...
	 case ButtonRelease:
		...
		if(xw_buttonpress(&event) == True) {
		    if (xim->exit_menu_flg && xim->j_c &&
			((event.xbutton.window ==
			  xim->cur_j_c_root->ichi->button[CANCEL_W]->window) ||
			 (event.xbutton.window ==
			  xim->cur_j_c_root->ichi->w1))) {
			syuuryou_menu();
		    } else {
			buff[0] = 0xd;
			ret = 1;
	 ...
	 case ClientMessage:
		...
		kill_xim(event.xclient.data.l[1], event.xclient.data.l[2],
...
#ifdef BC_X11R5
	    if (IS_NOT_FROM_XIMR5(event)) ximr5_client = 0;
#endif /* BC_X11R5 */
...
char *arg;
{
    ...
    if (w == cur_p->yes_no->w ||
	w == cur_p->yes_no->title->window ||
	w == cur_p->yes_no->button[0]->window ||
	w == cur_p->yes_no->button[1]->window) {
	return(True);
    }
    return(False);
}
