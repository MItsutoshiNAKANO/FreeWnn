/*
 *  $Id: atof.c,v 1.3 2001-06-14 17:55:37 ura Exp $
 */
...
#ifndef lint
static char *rcs_id = "$Id: atof.c,v 1.3 2001-06-14 17:55:37 ura Exp $";
#endif /* lint */
...
static void init(), read_attr(), read_id(), read_jiritugov(),
  ...
  print_out(), set_heap(), get_id_part(), check_attrs(),
  read_a_vector(), set_attribute(), bit_or(), set_id(), pre_clear_jiritugo_v(),
  usage(), sort_id();
...
check_fuzokugo()
{
  ...
  for(k = 1 ; k < fz_num;k++){
    if(wnn_Strcmp(fz[point].y,fz[k].y) == 0 &&
