/*
 * $Id: ujisf.c,v 1.1.1.1 2000-01-16 05:07:46 ura Exp $
 */
...
int (*func)();
{
	...
	if(func((char *)jep, (char *)prev) == 0){
	    ...
	    if (jt.syurui == CWNN_REV_DICT)
		fprintf(stderr,
"Entries with same yomi(%s), kanji(%s), hinsi(%s),sisheng(%s) are merged.\n",
		    tmp1, tmp2, wnn_get_hinsi_name((*jep)->hinsi),tmp3);
	    else
#endif
		fprintf(stderr,
"Entries with same yomi(%s), kanji(%s), hinsi(%s) are merged.\n",
