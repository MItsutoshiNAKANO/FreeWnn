/*
 *  $Id: dtoa.c,v 1.4 2001-06-14 17:55:37 ura Exp $
 */
...
#ifndef lint
static char *rcs_id = "$Id: dtoa.c,v 1.4 2001-06-14 17:55:37 ura Exp $";
#endif /* lint */
...
extern void parse_options(), usage(), input_dic(), udtoujis(), kset(),
  sdtoujis(), ghindo(), sort(), output_ujis(), exit1(), init_jeary(),
  get_kanji_str();
extern int input_header(), input_comment(), input_hinsi_list(),
  little_endian(), revdic(), wnn_loadhinsi(), init_heap(), w_stradd(),
  input_hindo_header(), motoni2(), get_n_EU_str();
#ifdef CONVERT_with_SiSheng
...
#ifdef CONVERT_with_SiSheng
    while ((c = getopt(*argc,*argv,"nseEzh:")) != EOF) {
#else
    while ((c = getopt(*argc,*argv,"nseEh:")) != EOF) {
#endif
	switch(c){
	case 'n': 
	    need_sort = 0;
...
char **argv;
{
	...
	if(bcmp(&file_head.file_uniq_org,&hjt.dic_file_uniq,
	   sizeof(struct wnn_file_uniq))){
