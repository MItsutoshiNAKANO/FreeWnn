/*
 *  $Id: atod.c,v 1.4 2001-06-14 17:55:37 ura Exp $
 */
...
#ifndef lint
static char *rcs_id = "$Id: atod.c,v 1.4 2001-06-14 17:55:37 ura Exp $";
#endif /* lint */
...
extern int wnn_loadhinsi(), init_heap(), little_endian(), Sorted(), asshuku(),
  revdic();
extern void ujis_header(), read_ujis(), reverse_yomi(), create_rev_dict(),
  uniq_je(), sort_if_not_sorted(), output_header(), udytoS(), output_ri(),
  exit1(), new_pwd(), rev_short_fun(), rev_w_char();
extern
...
static void ujistoud(), output_dic_index(), usage(), sdic_sort(),
  ujistosd(), not_enough_area(), get_pwd(), output_hindo(),
  output_hinsi(), output_kanji(), rev_dic_data(), set_pter1(),
  output_comment(), output_hinsi_list();
...
alloc_area()
{
  if(which_dict == WNN_STATIC_DICT){
      ...
      if((tary = (struct uind1 *)malloc(jt.maxserial * sizeof(struct uind1))) == NULL ||
                                /* Too large? */
	 (hostart =(UCHAR *)malloc(jt.maxserial *
		      (sizeof(struct uind2) + sizeof(w_char) * 2))) == NULL){
                                /* 2 must be enough? */
	  fprintf(stderr, "Malloc Failed\n");
	  exit(1);
      }
      hoend = (UCHAR *)((char *)hostart +
...
output_dic_index()
{
	...
	jt.hontai = hostart;	jt.table = tary;
	jt.ri1[D_YOMI] = NULL;
	jt.ri1[D_KANJI] = NULL;
	jt.ri2 = NULL;
#ifdef CHINESE
...
int *statep,*end_jep,*mid_jep;
{
  ...
  for(je = start_je;je < jt.maxserial && wnn_Strncmp(c,jeary[je]->yomi,level) == 0 &&
...
rev_dic_data()
{
    ...
    for(i = 0 ; i < jt.maxserial ; i++){
	rev_w_char((w_char *)(jeary[i]->kanji + 2),
		   *(jeary[i]->kanji) / 2 - 1);
    }
}
