/*
 *  $Id: jikouho_d.c,v 1.2 2001-06-14 17:55:36 ura Exp $
 */
...
static int cmp_dsd_dbn(), jkt_dbn(), jkt_sbjunjo(), set_jkt_dai(),
  zen_sbn(), cnt_par(), cnt_son();
	...
	*sb_que_head = sb_one->lnk_br;
	if(nmax > sb_one->kbcnt && yomi_eno > sb_one->j_c+1) {
	    ...
	    *sb_que_newcomer = NULL;
	    tmp = zen_sbn(sb_one->j_c+1, yomi_eno, sb_one->kangovect,
		      WNN_VECT_NO, sb_que_newcomer, sb_one->kbcnt + 1, sb_one
#ifndef	NO_KANA
			  , 0
#endif	/* NO_KANA */
			  );
...
/* ひらがな候補 */
    if (gijiflag == 0) {
	if ((sb_one = get_hira_kouho(&db_tmp, yomi_sno, yomi_eno))
					    < (struct JKT_SBN *)0)
	    return (-1);
...
/* カタカナ疑似文節を取り出します */
	...
	if (giji_sbn->hinsi_fk != katakanago_no &&
