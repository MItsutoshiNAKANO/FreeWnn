/*
 * $Id: do_filecom.c,v 1.1.1.1 2000-01-16 05:07:45 ura Exp $
 */
...
static int file_discard(), file_loaded_local(),
  find_file_by_uniq(), x_file_already_read(), add_fid_to_env(),
  del_fid_from_env(), new_fid_in_env(), file_remove();
...
js_file_comment_set()
{
    ...
    if(find_fid_in_env(envi,fid)==-1){ /* valid */
	wnn_errorno=WNN_FID_ERROR;
	goto ERR_RET;
    }
