/*
 *  $Id: prologue.c,v 1.3 2001-06-14 17:55:50 ura Exp $
 */
...
{
    for(;;){char x[256];
	printf(s); flush(); if(fgets(x, 256, stdin) == NULL){return(0);}
	if(*x=='n' || *x=='N') return(0);
	if(*x == 'y' || *x == 'Y')return(1);
    }
}
...
int first;
{
    ...
    if (cur_normal_env) save_cur_normal_env = cur_normal_env;
	...
	if (bun_data_ == NULL) {
            bun_data_ = jl_open_lang(environment, servername, lang_dir,
				     uumrc_name, yes_no_func,
				     puts_func, WNN_TIMEOUT);
	    ...
	    env_normal = jl_connect_lang(environment, servername,
					 lang_dir, uumrc_name,
					 yes_no_func, puts_func, WNN_TIMEOUT);
	}
	if (env_normal) {
	    if (normal_sticky)
    ...
    if (cur_reverse_env) save_cur_reverse_env = cur_reverse_env;
    for (p = reverse_env; p; p = p->next) {
		...
		strcpy(reverse_servername, def_reverse_servername);
            ...
            env_reverse = jl_connect_lang(environment, reverse_servername,
                                          lang_dir, uumrc_name, yes_no_func,
                                          puts_func, WNN_TIMEOUT);
            if(env_reverse) {
		if(reverse_sticky)
    ...
    if (save_cur_reverse_env) cur_reverse_env = save_cur_reverse_env;
    return(0);
}
