/*
 *  $Id: jhlp.c,v 1.3 2001-06-14 17:55:50 ura Exp $
 */
...
#ifndef lint
static char *rcs_id = "$Id: jhlp.c,v 1.3 2001-06-14 17:55:50 ura Exp $";
#endif /* lint */
...
static void do_end(), open_pty(), open_ttyp(), do_main(), exec_cmd(),
  parse_options(), setsize(), get_rubout(), usage(),
...
*/
      ml = (*code_trans[(internal_code << 2) | pty_c_flag])
...
unsigned char keyin0()
{
    ...
    for (;;) {
		...
		if (tty_c_flag == pty_c_flag) {
		    ...
		    i = (*code_trans[(tty_c_flag << 2) | file_code])
			(outbuf, buf, n);
		}
	    } else {
		i = (*code_trans[(tty_c_flag << 2) | file_code])
	...
	for(rfds = sel_ptn;
	  ...
	  ;
        if (sel_ret == 0) {
	    if ((tty_c_flag == J_JIS) &&
		((i = flush_designate((w_char *)outbuf)) > 0)) {
		/* 溜まっているＥＳＣを吐き出す */
		bufstart = outbuf;
		bufend = outbuf + i;
		return(*bufstart++);
...
*/
		...
		arrange_ioctl(1);
	    } else if (*buf == 0) { /* sequence of data */
		i = (*code_trans[(pty_c_flag<<2)|tty_c_flag])
		    (outbuf, buf+1, n-1);
#else /* defined(uniosu) */
